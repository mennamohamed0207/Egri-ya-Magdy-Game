...
namespace our {
    ...
    public:
        ...
        Sampler() {
            ...
            glGenSamplers(1, &name); // generates one sampler and saves it in the member variable "name"
        };

        // This deconstructor deletes the underlying OpenGL sampler
        ~Sampler() { 
            //TODO: (Req 6) Complete this function
            glDeleteSamplers(1,&name); // deletes one sampler that's saved it in the member variable "name"
         }

        // This method binds this sampler to the given texture unit
        void bind(GLuint textureUnit) const {
            //TODO: (Req 6) Complete this function
            glBindSampler(textureUnit,name); // bind the sampler "name" that we generated in the constructor to the same texture unit
        }

        // This static method ensures that no sampler is bound to the given texture unit
        static void unbind(GLuint textureUnit){
            //TODO: (Req 6) Complete this function
            glBindSampler(textureUnit,0); // bind the sampler "name" that we generated in the constructor to the same texture unit
        }
        ...
        void set(GLenum parameter, GLint value) const {
            //TODO: (Req 6) Complete this function
            glSamplerParameteri(name,parameter,value); //function version where the value is of type GLint
        }
        ...
        void set(GLenum parameter, GLfloat value) const {
            //TODO: (Req 6) Complete this function
            glSamplerParameterf(name,parameter,value); //function version where the value if of type GLfloat
        }
